...
def main():
    ...
    if args.robot_namespace != "":
        ...
        if laserscan_plugin is not None:
            tag_laserscan_ros_params = laserscan_plugin.find('ros')
            tag_laserscan_ns = ET.SubElement(tag_laserscan_ros_params, 'namespace')
            tag_laserscan_ns.text = '/' + args.robot_namespace + '/scan'
        else:
            print("ERROR>>>>>>>>>>>>>>>>>>>>> LASERSCAN NOT FOUND")
